...
#include "relativeMotionGgiFvMesh.H"
#include "Time.H"
...
namespace Foam
{
    defineTypeNameAndDebug(relativeMotionGgiFvMesh, 0);
    addToRunTimeSelectionTable(dynamicFvMesh, relativeMotionGgiFvMesh, IOobject);
}

// * * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * //

void Foam::relativeMotionGgiFvMesh::addZonesAndModifiers()
{
    // Add zones and modifiers for motion action

    if (cellZones().size() > 0)
    {
        Info<< "void relativeMotionGgiFvMesh::addZonesAndModifiers() : "
...
void Foam::relativeMotionGgiFvMesh::calcMovingMasks() const
{
    if (debug)
    {
        Info<< "void relativeMotionGgiFvMesh::calcMovingMasks() const : "
    ...
    {
        FatalErrorIn("void relativeMotionGgiFvMesh::calcMovingMasks() const")
...
Foam::relativeMotionGgiFvMesh::relativeMotionGgiFvMesh
(
    const IOobject& io
)
:
    ...
    movingPointsMaskPtr_(NULL)
{
    ...
    if (!cs_.inDegrees())
    {
        WarningIn("relativeMotionGgiFvMesh::relativeMotionGgiFvMesh(const IOobject& io)")
...
const Foam::scalarField& Foam::relativeMotionGgiFvMesh::movingPointsMask() const
{
    if (!movingPointsMaskPtr_)
    {
        calcMovingMasks();
...
bool Foam::relativeMotionGgiFvMesh::update()
{
