...
namespace Foam
{
    defineTypeNameAndDebug(multiRelativeFvMesh, 0);
    addToRunTimeSelectionTable(topoChangerFvMesh, multiRelativeFvMesh, IOobject);
}


// * * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * //

void Foam::multiRelativeFvMesh::addZonesAndModifiers()
{
    ...
    {
        Info<< "void multiRelativeFvMesh::addZonesAndModifiers() : "
        ...
        if (topoChanger_.size() == 0  && useTopoSliding())
        {
            FatalErrorIn
            (
                "void multiRelativeFvMesh::addZonesAndModifiers()"
...
bool Foam::multiRelativeFvMesh::useTopoSliding() const
{
    bool result = false;
...
void Foam::multiRelativeFvMesh::checkRotors() const
{
    ...
    forAll (topoChanger_, topoI)
    {
        if (isA<slidingInterface>(topoChanger_[topoI]))
        {
            ...
            if (rotorState != slider.attached())
            {
                FatalErrorIn
                (
                    "void multiRelativeFvMesh::checkRotors() const"
...
bool Foam::multiRelativeFvMesh::attached() const
{
...
Foam::multiRelativeFvMesh::multiRelativeFvMesh
(
    const IOobject& io
)
:
    ...
    attachDetach_(dict_.lookupOrDefault<bool>("attachDetach", true))
{
    ...
    //then set all relative rotors with its absolute sources
    forAll (rotors_, rotorI)
    {
      if(rotors_[rotorI].getRelativeRotor()=="none") continue;
      rotors_[rotorI].findAndSetOrigin(rotors_);
      //Foam::word originRotor(rotors_[rotorI].getRelativeRotor());
...
bool Foam::multiRelativeFvMesh::update()
{
    if (useTopoSliding())
    {
    ...
    vectorField startPhaseVector(allPoints().size(), vector::zero);

    forAll (rotors_, rotorI)
    {
        startPhaseVector += 
	  rotors_[rotorI].startPhasePointMotion();
    }
    ...
    // Move points
    movePoints(allPoints() + startPhaseVector);
    */
    // Accumulate point motion
    ...
    // Accumulate relative point motion
    vectorField relativePointMotion(allPoints().size(), vector::zero);

    forAll (rotors_, rotorI)
    {
      if(rotors_[rotorI].getRelativeRotor()!="none")
        relativePointMotion += rotors_[rotorI].relativePointMotion();
    }

    // Save old points
    oldPointsNew = allPoints();

    // Move points
    movePoints(allPoints() + relativePointMotion);
